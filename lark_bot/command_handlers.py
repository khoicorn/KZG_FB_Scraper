...
class CommandHandler:
    ...
    def handle_command(self, user_id, text):
        ...
        if text in ["help", "hi", "menu", "start", "hello"]:
            ...
            self.lark_api.reply_to_message(message_id, "❌ Sorry, I didn't understand. Type 'help' for options")
    ...
    def is_valid_domain(self, message_id, domain):
        ...
        if '@' in domain:
            self.lark_api.reply_to_message(message_id, "❌ Domain contains '@'. \nPlease provide a valid domain [e.g., chatbuypro.com]:")
            return False
        ...
        if not domain_pattern.fullmatch(domain):
            self.lark_api.reply_to_message(message_id, "❌ Invalid domain or domain is too short. \nPlease provide a valid domain [e.g., chatbuypro.com]:")
            return False

        # Additional optional check: min total length
        # if len(domain) < 8:  # e.g., x.co is 4 chars, a.com is 5
        #     self.lark_api.reply_to_message(message_id, "❌ Domain is too short. \nPlease provide a valid domain [e.g., chatbuypro.com]:")
        #     return False  
